...
\visible<4->{ &= \RootTwoN{n}\sum_{x=0}^{2^{n}-1} \ket{x} \\[1em]}
 \visible<5->{&= \RootTwoN{n}\sum_{x\in \Set{0,1}^{n}} \ket{x}}
\end{align*}
}
}
...
U_f\left(\TensProd{\RootTwoN{n}\sum_{x\in \Set{0,1}^{n}}\ket{x}}{\ket{-}}\right) &=
\RootTwoN{n}\sum_{x\in \Set{0,1}^{n}} U_f(\ket{x\,-}) \\
&= \RootTwoN{n} \sum_{x\in \Set{0,1}^{n}}\TensProd{\NegF{x}\ket{x}}{\ket{-}}
\end{align*}
\begin{itemize}
    \item Phase kickback places a sign on each term $x$ of the summation that is determined by $f(x)$.
\end{itemize}
