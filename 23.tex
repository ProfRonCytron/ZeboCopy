...
\lstick{\ket{\TensSupProd{0}{n}}} & \qwbundle{\alert<1>{n}}\slice{\alert<1>{\QState{0}}} &  \gate{\alert<3>{\TensSupProd{\Hadamard}{n}}}\slice{\alert<1>{\QState{1}}} & \gate[wires=2][5em]{\mbox{$U_f$}}\gateinput{$x$}\gateoutput{$x$}\slice{\alert<1>{\QState{2}}} &\gate{\alert<3>{\TensSupProd{\Hadamard}{n}}}\slice{\alert<1>{\QState{3}}} & \meter{0/1} \\
...
\begin{itemize}
    \item<4-> We know that
    \[ \TensSupProd{\Hadamard}{n}\ket{\TensSupProd{0}{n}} = 
    \RootTwoN{n}\sum_{x\in \Set{0,1}^{n}} \ket{x}
    \]
\end{itemize}
}
\end{frame}
