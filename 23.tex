...
\lstick{\ket{\TensSupProd{0}{n}}} & \qwbundle{\alert<1>{n}}\slice{\alert<8>{\QState{0}}} &  \gate{\alert<3,5>{\TensSupProd{\Hadamard}{n}}}\slice{\alert<9-10>{\QState{1}}} & \gate[wires=2][5em]{\mbox{$U_f$}}\gateinput{$x$}\gateoutput{$x$}\slice{\alert<11->{\QState{2}}} &\gate{\alert<3,6>{\TensSupProd{\Hadamard}{n}}}\slice{\alert<1>{\QState{3}}} & \meter{0/1} \\
...
\only<11>{%
\BigSkip{}
This is the phase-kickback trick result.}
}{%
\only<1-3>{%
\begin{center}
\Vskip{-3em}\adjustbox{valign=t, width=\textwidth}{\begin{quantikz}
...
\only<8->{%
\begin{align*}
   \only<8-10>{ \QState{0} &= \ket{\TensSupProd{0}{n}\,1} \\}
   \visible<9->{ \QState{1} &= \TensSupProd{\Hadamard}{n+1} \QState{0} \\}
    \visible<10-> {& =  \RootTwoN{n}\TensProd{\sum_{x\in \Set{0,1}^n}\ket{x}}{\ket{-}} \\}
    \visible<11->{\QState{2} &= \RootTwoN{n}\TensProd{\sum_{x\in \Set{0,1}^n} \NegF{x}\ket{x}}{\ket{-}}}
\end{align*}
}
}
\only<1-3>{%
...
\begin{itemize}
    ...
    \RootTwoN{n}\sum_{x\in \Set{0,1}^{n}} \ket{x} = \RootTwoN{n}\DQB{1}{\vdots}{\vdots}{1}
    \]
