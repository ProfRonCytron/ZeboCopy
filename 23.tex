...
\only<29->{%
\begin{itemize}
    \item<29-> If we measure \TensSupProd{0}{n} only, then $f(x)$ is constant.
    \item<30-> Otherwise we should never see \TensSupProd{0}{n} and $f(x)$ is balanced.
    \item<31-> What if \TensSupProd{0}{n} seen sometimes?
\end{itemize}
\MedSkip{}
}
}}%
...
\begin{itemize}
    \item<15-> \ColorThree{This is the exclusive-or of the \emph{and} of each pair of bits.}
    \item<16-> \ColorFour{This is the sum of the product of each pair of bits.}
\end{itemize}
}{%
\SmallSkip{}
We define the operation \DotP{v}{w} on two $n$-bit vectors $v$ and $w$ as follows:
\begin{align*}
   \visible<15->{ \DotP{v}{w} & \ColorThree{= \oplus_{i=1}^{n}\ {v_i}{w_i}} \\}
        \visible<16->{ & \ColorFour{= \sum_{i=1}^{n}\ {v_i}{w_i}\ \mbox{(mod 2)}}}
\end{align*}
\only<17->{\Vskip{-1em}These are equivalent (proof later).}
}}%
\only<17-23>{%
\[
P(n):  \TensSupProd{\Hadamard}{n}\ket{w}
  = \textcolor<23>{\RCthree}{\RootTwoN{n}\SumBV{v}{n} \NegOneExp{\DotP{v}{w}} \ket{v}}
\]}%
\only<18-23>{%
\begin{itemize}
\item<19-> \TensSupProd{\Hadamard}{n}\ket{\TensSupProd{0}{n}} provides the correct result, because \Implies{w=0^n}{\Forall{v}{\DotP{v}{w}=0}}.
\item<20-> We can use this formula to compute \QState{3}.
\end{itemize}}
\only<21-28>{%
\Vskip{-2em}\begin{align*}
\only<21-23>{%
    \visible<21->{\ColorOne{\sqrt{2^n}} \QState{3} &= \TensProd{\TensSupProd{\Hadamard}{n}
       (
       \ColorOne{\SumBV{w}{n} \NegF{w}\ket{w}}
       )}{\ColorTwo{\ket{-}}} \\}
    \visible<22->{   &= \TensProd{\ColorOne{\SumBV{w}{n} \NegF{w}} \TensSupProd{\Hadamard}{n}(\ColorOne{\ket{w}})}{\ColorTwo{\ket{-}}} \\}}
    \only<23-25>{\visible<23->{
    ...
    \visible<25->{%
    \only<26->{\ColorThree{\sqrt{2^n}}\ColorOne{\sqrt{2^n}}\QState{3}}&= \TensProd{\ColorThree{\SumBV{v}{n}} \ColorOne{\SumBV{w}{n}} \ColorOne{\NegF{w}} \ColorThree{\NegOneExp{\DotP{v}{w}}} \ColorThree{\ket{v}}}{\ColorTwo{\ket{-}}}
    }
\end{align*}
}%
\only<26->{%
Look at the $\ColorThree{\ket{v}}=\ket{\TensSupProd{0}{n}}$ term, where \Forall{\ColorThree{v}}{\ColorThree{\NegOneExp{\DotP{v}{w}}}=1}
    \begin{description}
      \item<27->[\Forall{w}{\ColorOne{f(w)}=0}:]  $\ColorOne{\SumBV{w}{n} 1}\ =\ {2^n}$. All other \ColorThree{\ket{v}} terms have zero amplitude.
      \item<28->[\Forall{w}{\ColorOne{f(w)}=1}:] $\ColorOne{\SumBV{w}{n} -1}\ =\ -{2^n}$. All other \ColorThree{\ket{v}} terms have zero amplitude.
      \item<28->[\ColorOne{$f(w)$} balanced:]
      \Vskip{-2em}\[
