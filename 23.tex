...
\begin{center}
\Vskip{-3em}\adjustbox{valign=t, width=\textwidth}{\begin{quantikz}
\lstick{\ket{\textcolor<5->{\RCone}{\TensSupProd{0}{n}}}} & \qwbundle{n}\slice{\alert<5>{\QState{0}}} &  \gate{\alert<2>{\TensSupProd{\Hadamard}{n}}}\slice{\alert<6-7>{\QState{1}}} & \gate[wires=2][5em]{\mbox{$U_f$}}\gateinput{$x$}\gateoutput{$x$}\slice{\alert<8>{\QState{2}}} &\gate{\alert<3-4,8>{\TensSupProd{\Hadamard}{n}}}\slice{\alert<9->{\QState{3}}} & \meter{0/1} \\
...
\item As drawn, \QState{2} includes only the top $n$ qubits.  
\item The complete state developed on output of $U_f$ is \TensProd{\QState{2}}{\ColorTwo{\ket{-}}}.
\item Still to come: how do we compute the \alert{\TensSupProd{\Hadamard}{n}} when its input is not \TensSupProd{0}{n}?
\end{itemize}}
}{%
\only<5-13>{%
...
\begin{itemize}
    ...
    \item<3-> But what about this one?  
    \item<4-> Its input is probably not \ket{\TensSupProd{0}{n}}, so we need to analyze what happens there.
\end{itemize}
}%
\only<11-13>{%
\BigSkip{}
...
\begin{frame}{Closer look at \QState{2}}{Case $f(x)$ is constant: \Forall{x}{f(x)=0}}
...
P(n):  \TensSupProd{\Hadamard}{n}\ket{\alt<3-4>{\alert{\TensSupProd{0}{n}}}{w}}
  = \RootTwoN{n}\SumBV{\ColorThree{v}}{n} \NegOneExp{\alt<5>{\ColorTwo{0}}{\ColorTwo{\DotP{v}{\alt<4>{0^n}{w}}}}} \ket{\ColorThree{v}}
\]

\only<1-6>{%
\begin{itemize}
\item<1-> We prove $P(n)$ later but accept it as true for now.
\item<2-> \TensSupProd{\Hadamard}{n}\ket{\alert{\TensSupProd{0}{n}}} provides the correct result, because \Implies{w=0^n}{\Forall{v}{\ColorTwo{\DotP{v}{w}=0}}}.
...
\only<7->{%
\TwoColumns{%
\begin{HTable}{101}{v}{3}
\visible<8->{\TE{000}{0}{+}{000} \\}
\visible<9->{\TE{001}{1}{-}{001} \\}
\visible<10->{\TE{000}{0}{+}{010} \\}
\visible<11->{\TE{001}{1}{-}{011} \\}
\visible<12->{\TE{100}{1}{-}{100} \\}
\visible<13->{\TE{101}{0}{+}{101} \\}
\visible<14->{\TE{100}{1}{-}{110} \\}
\visible<15->{\TE{101}{0}{+}{111}}
\end{HTable}
}{%
\begin{HTable}{011}{v}{3}
\visible<16->{\TE{000}{0}{+}{000} \\}
\visible<17->{\TE{001}{1}{-}{001} \\}
\visible<18->{\TE{010}{1}{-}{010} \\}
\visible<19->{\TE{011}{0}{+}{011} \\}
\visible<20->{\TE{000}{0}{+}{100} \\}
\visible<21->{\TE{001}{1}{-}{101} \\}
\visible<22->{\TE{010}{1}{-}{110} \\}
\visible<23->{\TE{011}{0}{+}{111}}
\end{HTable}
}}
\end{frame}
...
\only<5-9>{%
\BigSkip{}
\visible<5->{Notice what happens if we sum these tables.} \visible<6->{Constructive interference on term~\ket{00}.}  \visible<7->{Destructive interference on the the other three terms.}}}
\only<12->{%
...
\begin{center}
\Vskip{-3em}\adjustbox{valign=t, width=\textwidth}{\begin{quantikz}
\lstick{\ket{\TensSupProd{0}{n}}} & \qwbundle{n}\slice{\QState{0}} &  \gate{\TensSupProd{\Hadamard}{n}}\slice{\QState{1}} & \gate[wires=2][5em]{\mbox{$U_f$}}\gateinput{$x$}\gateoutput{$x$}\slice{\QState{2}} &\gate{\ColorThree{\TensSupProd{\Hadamard}{n}}}\slice{\QState{3}} & \meter{0/1} \\
...
\begin{frame}{Save Deutsch--Josza Solution}{After \QState{2}}
\only<7-8,16->{%
\Vskip{-4em}\TwoUnequalColumns{0.51\textwidth}{0.49\textwidth}{%
\begin{center}
\Vskip{-3em}\adjustbox{valign=t, width=\textwidth}{\begin{quantikz}
...
\begin{align*}
     \QState{2} &= \ColorOne{\RootTwoN{n}}\ColorOne{\SumBV{w}{n} \NegF{w}\ket{w}} \\
     \QState{3} &= \TensSupProd{\Hadamard}{n}(\QState{2})
\end{align*}
}
\only<16->{%
\begin{itemize}
...
\Vskip{-2em}\begin{align*}
\only<8-10>{%
    \visible<8->{\ColorOne{\sqrt{2^n}} \QState{3} &= \TensSupProd{\Hadamard}{n}
       (
       \ColorOne{\SumBV{w}{n} \NegF{w}\ket{w}}
       ) \\}
    \visible<9->{   &= \ColorOne{\SumBV{w}{n} \NegF{w}} \TensSupProd{\Hadamard}{n}(\ColorOne{\ket{w}}) \\}}
    \only<10-12>{\visible<10->{
    \ColorThree{\sqrt{2^n}}\ColorOne{\sqrt{2^n}}\QState{3} &= \ColorOne{\SumBV{w}{n} \NegF{w}}\ColorThree{\SumBV{v}{n} \NegOneExp{\DotP{v}{w}} \ket{v}} \\
    }}%
    \visible<12->{%
    \only<13->{\ColorThree{\sqrt{2^n}}\ColorOne{\sqrt{2^n}}\QState{3}}&= \ColorThree{\SumBV{v}{n}} \ColorOne{\SumBV{w}{n}} \ColorOne{\NegF{w}} \ColorThree{\NegOneExp{\DotP{v}{w}}} \ColorThree{\ket{v}}
    }
\end{align*}
}%
\only<13->{%
