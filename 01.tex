...
\begin{itemize}
    \item Given $z_{1}=\Polar{r_{1}}{\theta_{1}}$ and $z_{2}=\Polar{r_{2}}{\theta_{2}}$
    \item<2-> $z_{1}z_{2} = \Polar{r_{1}}{\theta_1}\Polar{r_{2}}{\theta_2}$
    \item<2-> $z_1z_2 = \Polar{r_{1}r_{2}}{(\theta_1+\theta_2)}$
    \item<3-> Given $a=\Polar{r}{\theta}$, how do we find $\Conj{z}$?
...
\\
 ...
 & = & \Prob{\QState{}}
\end{eqnarray*}
For a state~\QState{} that arises in a quantum system,  the above expression is the \emph{probability} of observing state \QState{} in that system.
\end{frame}
...
\begin{frame}{Unit vectors}{Probabilities must be preserved}
...
\Vskip{-3.5em}\begin{itemize}
    ...
    \]
    \item<3-> $p=1$ is the \href{https://en.wikipedia.org/wiki/Norm_(mathematics)\#Taxicab_norm_or_Manhattan_norm}{taxicab norm}, the sum of a vector of distances.
...
\end{itemize}}%
\OnlyRemark{6}{%
\href{https://scottaaronson.com/}{Scott Aaronson} has said that much of the wonder of quantum computing can be attributed to its use of the 2-norm.\LinkArrow{https://www.scottaaronson.com/democritus/lec9.html}
\MedSkip{}
Let's follow his way of thinking about this using the above arrow and see where it takes us.  And Scott Aaronson thinks and writes brilliantly.  I hope you enjoy reading his work as much as I do.
}
}{%
\[
