...
\begin{itemize}[<+->]
    \item Alice and Bob begin with $k$ EPR pairs, one for each bit of the key they wish to share.
    \item They can then separate and carry out the protocol.
    \item Detection of \href{https://en.wikipedia.org/wiki/Alice_and_Bob\#Cast_of_characters}{Eve, the eavesdropper} is accomplished by statistical analysis of most of their measurements.
    \item The protocol relies on the following property:
    \begin{itemize}
        \item Given an EPR pair, Alice and Bob will observe the same measurement if they measure in the same basis.  \alert{This is true no matter the basis.}
    \end{itemize}
    \item For this protocol, Alice measure in one of threee bases, and so does Bob.
    \item They publish the bases they used, and where they were the same, they should always agree on those measurements.
    \item Even where they differ, however, the bases used will cause the measurements to correlate more often than is classically possible.  If Eve is interfering, the correlation will drop to the classical level.
\end{itemize}
...
\begin{itemize}
    ...
    \item<2-> The basis $Z_\theta$ is rotated $\theta$ degrees counterclockwise from \ket{0}.
    \only<3>{%
    \item<3> While \ket{1} is similarly rotated, we will not show it to keep the diagrams simple and clean.}
    \item<4-> Here is the basis $Z_\frac{\pi}{8}$\visible<5->{, $Z_\frac{\pi}{4}$}\visible<6->{, and $Z_{-\frac{\pi}{8}}$.}
\end{itemize}}
\only<7->{%
\begin{itemize}
    \item<7-> \textcolor<7>{OrangeRed}{Alice} will measure in one of three bases.
    \item<8-> \textcolor<8>{OliveGreen}{Bob} will measure in one of three bases.
    \item<9-> With Bob and Alice each drawing from three bases, there are $9$ combinations of basis pairs.  \item<10-> Among those pairs, \textcolor<10>{Purple}{$2$ have the same basis} for both Alice and Bob.
\end{itemize}
}
}{%
\begin{center}
\begin{TIKZP}[scale=2]
\textcolor<8>{OliveGreen}{%
\textcolor<7>{OrangeRed}{%
\textcolor<10>{Purple}{%
...
\draw<2-3>[->] (0,0) -- (30:1);
\draw<3>[->,dotted] (0,0) -- (120:1);
\node<2-3> at (0.7,0.2) {$\theta$};
\textcolor<7>{OrangeRed}{%
\draw<5->[->,thick] (0,0) -- (45:1) node[right] {$\frac{\pi}{4}$};
\textcolor<8>{OliveGreen}{%
\textcolor<10>{Purple}{%
\draw<4->[->,thick] (0,0) -- (22.5:1) node[right] {$\frac{\pi}{8}$};}%
}%
}
\textcolor<8>{OliveGreen}{%
\draw<6->[->,thick] (0,0) -- (-22.5:1) node[right] {$-\frac{\pi}{8}$};}
\end{TIKZP}
\end{center}
}
