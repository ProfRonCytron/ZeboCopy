...
\def\CDa{\alt<3->{\alt<7->{\CFCirc{\RCthree}}{\CFCirc{\RCtwo}}}{\CFCirc{\RCone}}}
\def\CDb{\alt<3->{\alt<7->{\CFCirc{\RCfour}}{\CFCirc{\RCtwo}}}{\CFCirc{\RCone}}}
\def\CDc{\alt<3->{\alt<8>{\CFCirc{\RCone}}{\CFCirc{\RCtwo}}}{\CFCirc{\RCone}}}
\def\CDd{\alt<3->{\alt<8>{\CFCirc{\RCtwo}}{\CFCirc{\RCtwo}}}{\CFCirc{\RCone}}}
...
\Vskip{-3em}\begin{itemize}
    ...
    \item<6-> However, the matrix \ColorTwo{can act differently on basis states~\ket{\TensSupProd{1}{n-1}0} and \ket{\TensSupProd{1}{n-1}1}}.
...
\only<9->{%
\begin{itemize}
    \item<9-> Thus, when the input's left $n-1$ bits are all~$1$, we can act on the rightmost bit applying the unitary gate \SQBG{\relax}{\CFCirc{\RCthree}}{\CFCirc{\RCone}}{\CFCirc{\RCfour}}{\CFCirc{\RCtwo}}.
    \item<10-> This is a \emph{conditional} gate and can be represented by the circuit:
\end{itemize}
    \visible<10->{\adjustbox{scale=0.8,valign=t}{%
\begin{quantikz}
    \lstick{$q_1$} & \qw \\
    \lstick{$q_2$} & \qw \\
    \lstick{$\RVDots$} & \qw  \\
