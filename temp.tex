...
\TwoColumns{%
\begin{itemize}
    \item We begin with a quantum gate for an~$n$~qubit system.
    \item Conceptually this can be viewed as a~$2^{n}\times2^{n}$ matrix~$U$.
    \item We apply transformations to $U$, each of which operates only on 2~qubits at a time, to reduce $U$ to a form where it also operates only on $2$~qubits.
\end{itemize}
}{%
\only<2>{
\[2^{n}\begin{pmatrix*}
 ...
 \CC & \CC &  \CC & \cdots& \CC \\
 \CC & \CC &  \CC & \cdots& \CC \\
 \RVDots & \RVDots & \RVDots  & \RVDots & \RVDots \\
 \CC & \CC & \CC & \cdots& \CC
\end{pmatrix*}\]}
}
\end{frame}}
