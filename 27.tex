...
\begin{itemize}
    ...
    \item In each complete run of Grover, the probability of \emph{not} measuring \W{} each time is $0.99$.
    \item If we run Grover $k$ times, then the probability of not seeing \W{} in any of those runs is $\left(\frac{99}{100}\right)^{k}$
    \item The probability of measuring \W{} sometime in $k$ runs is then \( 1-\left(\frac{99}{100}\right)^{k} \)
    \item If we perform $110$~runs then we measure \W{} with approximate probability~$\frac{2}{3}$
\end{itemize}
\end{frame}
