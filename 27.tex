...
\begin{frame}{Steps of the algorithm}{Factoring 217 using a base of 11 this time.}
\Vskip{-4em}\TwoUnequalColumns{0.5\textwidth}{0.5\textwidth}{%
\begin{itemize}
    \item<1-> \alert{Find the period} \ColorFive{$r$} of $\ColorFour{b}^{x}\SMod{N}$.
    \item<2-> If the period is not \emph{even}, abort.
    \item<3-> Write $\ColorFour{b}^{\ColorFive{r}}$ as $\left(\ColorFour{b}^{\ColorFive{r}/2}\right)^{2}$
    \item<4-> If $\ColorFour{b}^{\ColorFive{r}/2}=\pm 1\SMod{N}$, abort
    \item<5-> Factor $\left(\ColorFour{b}^{\ColorFive{r}/2}\right)^{2}-1$ into $\ColorOne{\left(\ColorFour{b}^{\ColorFive{r}/2}-1\right)}\times \ColorTwo{\left(\ColorFour{b}^{\ColorFive{r}/2}+1\right)}$
    \item<6-> Factors are
    \begin{itemize}
...
\begin{itemize}
    \item<1-> The period of $\ColorFour{11}^{x}\SMod{217}$ is~\ColorFive{$30$}.
    \item<2-> \ColorFive{$30$} is even, so we are good.
    \item<3-> $\ColorFour{11}^{\ColorFive{30}} = \left(\ColorFour{11}^{\ColorFive{15}}\right)^{2}$
    \item<4-> $\ColorFour{11}^{\ColorFive{15}}=92\SMod{217}$, so we are good.
    \item<5-> $\ColorFour{11}^{\ColorFive{30}}-1 =\ColorOne{\left(\ColorFour{11}^{15}-1\right)}\times\ColorTwo{\left(\ColorFour{11}^{15}+1\right)}$
    \item<6-> Factors are
    \begin{itemize}
...
\BigSkip{}
Throughout, \emph{abort} requires starting over with a new (guessed) value for the base.
