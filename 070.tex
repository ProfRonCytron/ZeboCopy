\SetTitle{7}{Universal quantum gate for a single qubit}{Several approaches, culminating in the qiskit U gate}{07}
...
\setcounter{enumi}{3}
   ...
   \[\ket{\QName{1}\QName{2}\cdots\QName{n}}\]
\end{enumerate}}
\end{frame}
...
\begin{itemize}
    \item<3-> $\Mag{\ExpPhase{\phi}}=1$ for all $\phi$
...
\end{frame}
\begin{frame}{Conclusion of Proof}{We now have a general unitary matrix specified using only three real parameters}
...
\begin{frame}{Universality by a sum of Pauli gates}{This material is optional}
\begin{itemize}
    ...
    \item Moreover, we already have universality using the three real, scalar parameters of the \NamedGate{U} gate.
