\SetTitle{15}{Universal quantum gates}{Exact and approximate}{15}
...
\Vskip{-3.5em}\begin{itemize}[<+->]
    ...
    \item The (column-vector) representation of an $n$-qubit quantum system grows exponentially in the number of qubits.
    \item Some of those systems are \href{https://en.wikipedia.org/wiki/Quantum_entanglement}{entangled}, requiring a separate accounting of each of the~$2^n$ possible measurements.
...
\only<13->{%
\begin{itemize}
    \item<13-> Can quantum circuits be similarly created?  For example$\ldots$
    ...
    \lstick{$q_3$} &\qw & & \qw
    \end{quantikz}}
    \item<16-> Recall previously we proved we can entangle a new qubit with some old ones using only a (two-input) \NamedGate{CNOT} gate.
    \item<17-> More generally, what kinds of components are needed to develop a programmable quantum device?
\end{itemize}}
\end{frame}
...
\begin{description}
\item<1->[no superposition]  The gate set \Set{\NamedGate{CNOT}} cannot create superposition on its own.  The gate (matrix) simply maps basis states to other basis states. \only<1>{\begin{center}\CNOTMatrix\end{center}}
