...
{
\def\TB#1#2{\LBP{}\ColorOne{#1}\ColorTwo{#2}}
...
\begin{center}
    \begin{tabular}{cc}
    \LBP{}\WO\WT & $w$ \\ \hline
    \visible<6->{\TB{0}{0} & 0 \\[0.2em]}%
    \visible<7->{\TB{0}{1} & $\frac{1}{4}$\\[0.2em]}%
    \visible<8->{\TB{1}{0} & $\frac{1}{2}$ \\[0.2em]}%
    \visible<9->{\TB{1}{1} & $\frac{3}{4}$}
    \end{tabular}
\end{center}
}
}{%
...
\only<10->{%
\begin{description}
    \item<10->[$\Hadamard(\ColorThree{top}) = \ket{0}$] then $\WT=0$ and we determine \WO{} by $\Hadamard(\ColorFour{bottom})$, yielding $w=0$ or~$\frac{1}{2}$.
    \item<11->[$\Hadamard(\ColorThree{top}) = \ket{1}$] then we rotate away this bit and measure $\Hadamard(\ColorFour{bottom})$, yielding $w=\frac{1}{4}$ or~$\frac{3}{4}$.
\end{description}
