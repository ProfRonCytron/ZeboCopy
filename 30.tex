...
\begin{itemize}[<+->]
    \item Analysis shows us that each qubit reveals another binary digit of our phase $w$, starting with the least significant bit.
    \item Given a qubit that corresponds to
    \[ \LBP{}\ColorTwo{w_{k+1}}\ColorThree{w_{k+2}w_{k+3}\cdots w_{n}} \]
    we isolate \ColorTwo{$w_{k+1}$} by rotating away \ColorThree{any bit afterwards} that has value~$1$.
