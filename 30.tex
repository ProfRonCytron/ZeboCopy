\SetTitle{30}{Inverse Fourier transform}{Finding the phase difference among superpositions in a periodic state}{30}
...
\begin{itemize}[<+->]
    \item The inverse Fourier transform is needed to complete estimation of the phase present after applying a unitary gate to one of its eigenstates.\LinkArrow{https://en.wikipedia.org/wiki/Quantum_phase_estimation_algorithm}
...
\visible<4>{
      \TZPoint{0,1}{$\ExpPhase{\pi/2}=\NiceI$}{above right}
...
\visible<6>{%
      \TZPoint{0,-1}{$\ExpPhase{3\pi/2}=-\NiceI$}{below right}
...
\begin{itemize}
    ...
    \item<4-> The basis states all have the same magnitude~\textcolor<4>{\RCtwo}{\RootTwoN{n}}, so each is equally likely upon measurement.
\end{itemize}
...
\visible<7>{\alert{hey do this horizontally also}}
\end{frame}}
...
\only<13>{
\begin{itemize}
  \item How do we realize the $R$ gates?  This is left as an exercise.
\end{itemize}
}
\end{frame}}
...
\begin{itemize}
    ...
    \item<6-> To rotate the $k$th bit from the binary point, use (\href{https://qiskit.org/textbook/ch-states/single-qubit-gates.html\#rzgate}{qiskit gate}):
...
\Reason{10}{Push the \ColorThree{\QZero{}} and \ColorThree{\QOne{}} into the \ColorOne{summation}}%
\Reason{11}{Factor out the \ColorOne{summation}}
\Reason{14}{\ColorFive{Combine} \ColorThree{this exponent} with \ColorOne{its adjacent exponent}}%
