...
\item The named axes are mutually orthogonal (exercise).
...
\TwoColumns{%
\begin{itemize}
    \item Every pair of antipodal points determines an orthogonal basis for a the quantum state of a single qubit.
...
\begin{itemize}
    ...
    \item The gates we study here are quantum gates, used in the middle of a quantum circuit.  There are equivalently named \emph{measurement} gates, used near the end, that we study later.  They model collapse under measurement, are nonunitary, and are thus different from the ones we study here.
\end{itemize}
...
\]
\begin{itemize}
    \item<1-> This matrix transforms a state as follows: \[ \ApplyGate{\PauliZ}{\SQB{\alpha}{\beta}} \mapsto  \SQB{\alpha}{-\beta} \]
...
\]
\begin{itemize}
    \item<1-> This transformation exchanges the amplitudes: \[ \ApplyGate{\PauliX}{\SQB{\alpha}{\beta}} \mapsto  \SQB{\beta}{\alpha} \]
    \item<2-> The two unaffected states are \ket{+x} and \ket{-x} (exercise).
...
\]
\begin{itemize}
    \item<1-> This transformation exchanges the amplitudes and imposes a relative sign change: \[ \ApplyGate{\PauliY}{\SQB{\alpha}{\beta}} \mapsto  \SQB{-i\beta}{i\alpha} = -i\SQB{\beta}{-\alpha} \equiv \SQB{\beta}{-\alpha} \]
    \item<2-> The two unaffected states are \ket{+y} and \ket{-y} (exercise).
    \item<3-> This gate maps $\ket{0}\mapsto\ket{1}$ and \VV{}.
\end{itemize}
}{%
\begin{center}
