...
\only<12-14>{%
\begin{itemize}
    \item<12-> It is misleading to see the \alert<12>{output} of $U_f$ labeled to look like a tensor product.
    \item<13-> Without analysis, we cannot know if the output can be tensor factored.
    \item<14-> Consider \textcolor{\RCone}{$f(0)=0$} and \textcolor{\RCtwo}{$f(1)=1$}.  Then $U_f$ creates \alert<14>{\TwoSup{00}{11}} as the output, which we know cannot be tensor factored!
\end{itemize}
}%
\only<15->{%
\begin{itemize}
