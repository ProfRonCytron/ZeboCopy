...
\TwoColumns{%
\begin{itemize}
    \item<7-> Each Pauli matrix is an observable.
    \item<8-> The \PauliZ{} matrix corresponds to the \href{https://en.wikipedia.org/wiki/Qubit\#Standard_representation}{computational basis}, because its eigenvectors are \alert<8>{\ket{0}} \visible<9->{and \alert<9>{\ket{1}}.}
    \item<10-> Starting from an \alert<10>{arbitrary state~$\psi$}, \visible<11->{once measured, subsequent imposition of \PauliZ{} produces the same eigenvalue and eigenvector.}
\end{itemize}
}{%
\only<8-11>{%
\begin{align*}
    \ZMatrix{} \alert<8>{\PZero{}} & = & +1  & \alert<8>{\PZero{}}  \\
    \visible<9->{\ZMatrix{}  \alert<9>{\POne{}} & = & -1  & \alert<9>{\POne{}} }
\end{align*}}
\begin{center}
\begin{TIKZP}
\visible<8->{
...
\visible<10>{\draw[->,thick] (0,0) -- (30:1) node[above] {\alert<10>{$\psi$}};}
}
