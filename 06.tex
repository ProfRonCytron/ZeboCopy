...
\begin{itemize}
    \item<1-> To unrotate, sending \QZero{} to $\ket{+}$ and \QOne{} to $\ket{-}$, we use the familiar Hadamard matrix:
    \[ \Hadamard{}=\HMatrix{} \]
    \item<4-> To rotate, we simply compute $\Conj{\Hadamard}=\Hadamard{}$.
    \item<4-> To summarize, we apply $\Conj{T}$, perform the measurement in the standard basis, and then apply $T$.
\end{itemize}
}
\end{frame}
%%
