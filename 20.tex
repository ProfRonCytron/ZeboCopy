...
\begin{frame}{Oracles}{Boxes that provide answers}
...
\begin{itemize}[<+->]
  ...
  \begin{itemize}
      \item It must respect all promises it makes.
...
\section*{Realizing classical gates}

{
\def\AlpZer{\ColorOne{\mbox{$\alpha_0$}}}
\def\AlpOne{\ColorTwo{\mbox{$\alpha_1$}}}
\def\BetZer{\ColorThree{\mbox{$\beta_0$}}}
\def\BetOne{\ColorFour{\mbox{$\beta_1$}}}
...
The result
{\small\[
\AlpZer{}\BetZer{}\ket{00} +
  \AlpZer{}\BetOne{}\ket{01} +
  \AlpOne{}\BetOne{}\ket{10} +
  \AlpOne{}\BetZer{}\ket{11}
\]}%
...
\Vskip{-3em}\begin{itemize}
  \item<11-> Consider \CNOT{a}{b} with $a=\AlpZer{}\ket{0}+\AlpOne{}\ket{1}$ and $b=\BetZer{}\ket{0}+\BetOne{}\ket{1}$
  \item<11-> The input is the tensor product
  $\AlpZer{}\BetZer{}\ket{00} +
  \AlpZer{}\BetOne{}\ket{01} +
  \AlpOne{}\BetZer{}\ket{10} +
  \AlpOne{}\BetOne{}\ket{11}
  $
  \item<12-> So we compute {\small
  \[ \CNOTMatrix{}\DQB{\AlpZer{}\BetZer{}}{\AlpZer{}\BetOne{}}{\AlpOne{}\BetZer{}}{\AlpOne{}\BetOne{}}=\DQB{\AlpZer{}\BetZer{}}{\AlpZer{}\BetOne{}}{\AlpOne{}\BetOne{}}{\AlpOne{}\BetZer{}} \]}
\end{itemize}
}
}
...
Each column is the output produced when given its corresponding input basis vector.
}{%
...
\only<5-9>{%
\BigSkip{}
We can express those row vectors as bras and the column vectors as kets.
}
}{%
\begin{align*}
    U & =
...
\[
\textcolor{\RCthree}{%
\DQB{0}{0}{1}{0}\mapsto \DQB{0}{0}{0}{1}
}
\]
}
\only<5>{%
...
\begin{frame}{Summary}{Where are we going?}

\begin{itemize}
    \item Quantum gates provide elements to create circuits that realize classical functions of inputs.
    \item Such logic will be embedded in an oracle, but we will not see the logic directly.
    \item We study a series of problems that discover properties of the oracle without seeing the logic within.
    \item While we can conceive of the oracle's logic in terms of matrices and unitary gates, the size of a matrix grows exponentially in the number of qubits.
    \item We must therefore consider more direct ways of deploying logic to realize oracles for testing of our algorithms.
\end{itemize}
