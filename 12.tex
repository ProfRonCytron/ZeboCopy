...
\begin{itemize}
    ...
    \item<4-> \RD{Alice}'s basis is rotated $\pi/$ radians clockwise from~\ket{0}.
\end{itemize}}%
\only<5-6>{%
\begin{itemize}
    \item<5-> Consider \GN{Bob}'s other possibility.
...
\draw<1,7->[->,thick] (0,0) -- (0,1) node[above] {\ket{1}};
\draw<2-4,7->[->,thick,\Orange] (0,0) -- (0:1) node[right] {Bob};
\draw<2>[->,thick,dotted,\Orange] (0,0) -- (90:1) node[above] { };
\draw<4,6,7->[->,thick,\Red] (0,0) -- (-22.5:1) node[right] {Alice};
\draw<3-5,7->[->,thick,\Yellow] (0,0) -- (22.5:1) node[right] {Alice};
\draw<5-,7->[->,thick,\Green] (0,0) -- (45:1) node[above right] {Bob};
\end{TIKZP}
\end{center}
\BigSkip{}
\only<2>{%
We show only the \ket{0}~axis of a basis.  The \ket{1}~axis is $\pi/2$~radians counterclockwise.
\BigSkip{}
If \OR{Bob} measures first, he is equally likely to measure~$0$ or~$1$.
}%
\only<3>{%
...
\only<4>{%
The same is true here:  the probability that \RD{Alice} and \OR{Bob} measure the same result is about~$0.85$, no matter who measures first.
}%
\only<5>{%
If \GN{Bob} measures first, he will surely see~$0$.  But then \YL{Alice} is about $85\%$ likely to see~$0$ after that.  If \YL{Alice} measures first, she will likely measure~$0$.  With either ensuing change of state, \GN{Bob} is $85\%$ likely to match \YL{Alice}'s measurement.
}
\only<6>{%
The probability of \emph{disagreement} between \RD{Alice} and \GN{Bob} is approximately~$0.85$, no matter who measures first.
}%
}%
\only<10->{%
\BigSkip{}
The quantum approach wins $\sim 85\%$ of the time, while classically the best we achieve is to win $75\%$ of the time. \alert{Quantum behavior provides a clear and measurable advantage over classical logic.}
