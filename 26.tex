...
\only<7->{%
Consider a two-qubit example with $f(x)=1$ only when $x=2$.  Then we have:
\begin{align*}
    \W &= \Conj{\CDQB{0}{0}{1}{0}}\\
    \S &= \Conj{\RootTwoN{2}\CDQB{1}{1}{1}{1}}\mbox{ (always)} \\
    \visible<8->{\R &= \Conj{\RootN{3}\CDQB{1}{1}{0}{1}} \\}
    \visible<9->{\cos{\theta} &= \braket{\ColorOne{s}}{\ColorTwo{r}}=\frac{\sqrt{3}}{2} \\
    \theta &= \frac{\pi}{6}
    }
\end{align*}}
}{%
...
\begin{TIKZP}[scale=3]
    \visible<1->{\draw[->,thick,\RCone] (0,0) -- (30:1) node[right]{\S};} \visible<2->{\draw[->,thick,\RCthree] (0,0) -- (0,1) node[above]{\W} ;}
