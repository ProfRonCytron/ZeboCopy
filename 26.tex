...
\Vskip{-3em}\begin{itemize}
    ...
    \item<3-> If instead we want to trigger the change on the \ket{0} state, we need only complement the incoming but using an \PauliX{} gate, as shown for $x_2$ and $x_3$.  
    \item<4-> We must restore those bits after the \NamedGate{mcx} gate.
\end{itemize}
}{%
\visible<2->{%
\begin{center}
...
\only<3>{\lstick{$x_2$} & \gate{X} & \ctrl{1}  & \qw & \qw \\}
\only<4->{\lstick{$x_2$} & \gate{X} & \ctrl{1}  & \gate{X} & \qw \\}
\only<2>{\lstick{$x_3$} & \qw & \ctrl{1}  & \qw & \qw \\}
\only<3>{\lstick{$x_3$} & \gate{X} & \ctrl{1}  & \qw & \qw \\}
\only<4->{\lstick{$x_3$} & \gate{X} & \ctrl{1}  & \gate{X} & \qw \\}
\lstick{$x_4$} & \qw & \ctrl{1} & \qw & \qw \\[1em]
\lstick{$y$}   & \qw  & \targ{} & \qw & \qw \\
\end{quantikz}}
\end{center}}
}
