...
\Vskip{-4em}\begin{align*}
    \ColorOne{\ket{s}} &= \TensSupProd{\left(\PPlus\right)}{n} = \RootTwoN{n}\SumBV{x}{n}\ket{x} \\
    \visible<2->{\ColorThree{\ket{w}} &= \frac{1}{\sqrt{\Mag{T}}}\sum_{t \in T} \ket{t} \\}
    \visible<6->{\ColorTwo{\ket{r}} &= \frac{1}{\sqrt{2^{n}-\Mag{T}}}\sum_{t \not\in T} \ket{t}}
\end{align*}
...
\begin{TIKZP}[scale=3]
    \visible<1->{\draw[->,thick,\RCone] (0,0) -- (15:1) node[right]{\ket{s}};} \visible<2->{\draw[->,thick,\RCthree] (0,0) -- (0,1) node[above]{\ket{w}} ;}
    \visible<6->{\draw[->,thick,\RCtwo] (0,0) -- (1,0) node[right] {\ket{r}};}
\end{TIKZP}
\end{center}}
\only<1>{We begin with the superposition of all inputs $\ColorOne{\ket{s}}=\TensSupProd{\Hadamard}{n}\left(\TensSupProd{0}{n}\right)$}%
\only<2>{%
Let $T=\Set{t\ | n\ f(t)=1}$.   Measurement of \ColorThree{\ket{w}} obtains a solution to the problem.}
