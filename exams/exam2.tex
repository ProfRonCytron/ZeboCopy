...
Below, the term \emph{exactly} means that we are guaranteed a solution in the specified time.
\begin{itemize}
    \item Deutsch--Jozsa solves a problem in polynomial time on a quantum computer that takes exponential time on a classical computer to solve exactly.~\TF{}
    \item With a single query, a classical computer can solve Deutsch--Jozsa problems half the time.~\TF{}
    ...
    \item If the Deutsch--Jozsa quantum algorithm is presented with an oracle from Bernstein--Vazirani, then the circuit's measurements will yield a unique result in the computational basis.~\TF{}
