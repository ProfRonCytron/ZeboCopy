...
\item \Points{10} To make grading easier please use the format below, or if you write your answers out on your own, please imitate the format of the response section.

\textbf{Questions (do not respond here---see response section below)}
\begin{enumerate}[label=\theenumi.\arabic*]
   \item\label{tf:a}\Points{0} The university course evaluation for this course is worth one of the five points for
...
the correct solution to a Deutsch–Jozsa problem.
    \item\label{tf:j} \Points{2} For the work of a Variational Quantum Eigensolver carried out on a quantum computer, why is noise generally considered unproblematic?
       \item\label{tf:b}\Points{2} How does Shor's algorithm find the period of a function?
\end{enumerate}
...
\ref{tf:a} \textbf{true} \ \ \TFPrompt{d}\TFPrompt{e} \TFPrompt{f}

\TFPrompt{g}\TFPrompt{h}\TFPrompt{i} \\[1.2em]
\ref{tf:j}~\Blank[5in]{} \\[1.2em]
\ref{tf:b}~\Blank[5.75in]{} \\[1.2em]
\Blank[6in]{} \\[1.2em]
\Blank[6in]{}

\item\Points{15}  We have studied the 3-qubit correction scheme that can correct a single \PauliX{} error on one of 3 entangled qubits.  To achieve this, we conducted \emph{syndrome} measurements of those three qubits onto two other qubits.

The qubits receiving the syndrome values are expected to be in state \QZero{} or \QOne{}, and the combined value of the two syndrome qubits indicates what, if any, corrective action must be taken on the 3 entangled qubits.
...
\begin{enumerate}[label=\theenumi.\arabic*]
  ...
  \LeaveSpace{}
  \item Based on our studies, what is the best way to reset a syndrome qubit back to \QZero{} after its measurement has been taken?  Assume the syndrome qubit is qubit $\ket{s}$ and its measurement has been taken on classical bit $c$ (diagram below).  
  \begin{itemize}
      \item Describe your approach:\LeaveSpace{}
      \item Draw your approach on the circuit below.  The diagram below is intended to show a measurement of qubit $\ket{s}$ onto classical bit $c$, the way \texttt{qiskit} does measurements.  It is \emph{not} a controlled measurement: it is always performed.
  \end{itemize}
\begin{center}
\adjustbox{valign=t}{\begin{quantikz}
\lstick{$\ket{s}$}&  \qw & \ctrl{1}&\qw & \qw & \qw & \qw & \qw\rstick{should now be \QZero{}} \\[4em]
\lstick{$c$} &   \qw    &  \meter{}& \qw &\qw & \qw & \qw & \qw
\end{quantikz}}%
\end{center}
\end{enumerate}
...
\begin{center}
    ...
    \begin{center}
        ...
        \end{tabular}
    \end{center}}
    \clearpage\item  The qubits start again in state $\ket{\psi}$.  Below is a depiction of the Mermin--Peres square with its measurement operators.  For each square, circle it if, when Alice and Bob have that square in common, they are \emph{guaranteed} to report the same value.  Again, with some thought, this is easily answered, but you can also grind through all nine squares.

    \[\includegraphics[scale=0.8]{mp}\]
        What is the basis for your above answers?\LeaveSpace{}

        \item The qubits start again in state $\ket{\psi}$.  Below is a another depiction of the Mermin--Peres square with its measurement operators.  For each square, circle it if, when Alice and Bob have that square in common, they \emph{might} (probability greater than zero) report the same value.  Again, with some thought, this is easily answered, but you can also grind through all nine squares.
    \[\includegraphics[scale=0.8]{mp}\]
    \end{enumerate}

\clearpage\item\Points{5} In class we studied how the Quantum Fourier Transform can break Diffie--Hellman by efficiently solving the discrete log problem.  The videos related to that material can be found \href{https://www.youtube.com/watch?v=PV0d7vCHKN4}{here} and \href{https://www.youtube.com/watch?v=c1WRcu6Fy0A}{here}.
...
\clearpage\item\Points{10}  In this problem you will consider solving Simon's problem using Grover's algorithm.  In this problem, the domain values for the Simon function are $n$~qubits.  You are given the following components:
\begin{itemize}
...
\item\Points{15}  In class we experimented with instances of the Bernstein--Vazirani problem with the goal of understanding where noise is present.  Based on the results presented in class, and the published solutions submitted by students, answer the following questions.
...
\item\Points{15} The IBM Quantum Computer offers a two-qubit instruction \href{https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.ECRGate}{\NamedGate{ECR}} (Echoed Cross-Resonance) that is defined as follows (there are other definitions, but for the exam, use this one):
\[
