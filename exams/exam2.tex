...
\lstick{$q_0$} &  \qw & \qw & \qw & \qw & \qw & \qw  \\[2em]
...
\lstick{$q_0\mbox{ in state }\ket{\psi}$}& \qw& \qw & \gate[2]{E} & \qw & \qw & \qw & \qw  & \qw & \qw & \qw & \qw \\[0.5em]
...
Suppose Alice and Bob decide to form a shared key using the BB84 protocol.  The table below shows the bases they will publish along with their observed results.   Recall the correspondence between observed quantum state and bit values:
\begin{BBKey}
\begin{center}
\BBBasis{}
\end{center}
...
\item\Points{10} The analysis of Grover's algorithm in class assumed that there was exactly one input for which our oracle returned~$1$; on all other inputs, the oracle returned~$0$.  Succinctly described, we say the oracle \emph{yields} exactly one solution.   
...
\begin{enumerate}
    \item Consider the expression  \[ \sqrt\frac{2^{n}-1}{2^{n}}\]
     ...
     \LeaveSpace{0.75in}
     \item To make the following answers easier to write and grade, assume a suitable scaling factor has been pulled out of a quantum state, so that the resulting ket can be described in terms of only its $1$s and $0$s.  For example:
     \begin{itemize}
  ...
  For an $n$-qubit instance of Grover, describe the state \ket{s} when 
        ...
        \end{enumerate}
    Now consider the ket \ket{r} from the slides.  Be sure you are using \ket{r}, the horizontal axis, and not \ket{w}, the vertical axis.  In the same style with which you should have characterized \ket{s}, now describe \ket{r} for an $n$~qubit instance of Grover when
        \begin{enumerate} 
           \item the oracle yields one solution~\Blank[2.5in]{}
           \item the oracle yields $k$ solutions~\Blank[2.5in]{}
        \end{enumerate}
       \item In a world where Grover meets Deutsch--Jozsa (this is possible: both algorithms are from the 1990s), consider the use of an oracle that promises that exactly one of the following is true of its function $f(x)$ where $x$ is $n$-qubits wide:
       \begin{description}
...
\item\Points{10}
Consider the circuit below, which tests whether the input $x$ is the binary string ``00111'', each 5 bits wide.  Suppose we have reason to believe that the top qubit is faulty in the sense that it behaves as if it were subjected to an \X{}~gate, just prior to being used as a control bit for the multi-controlled \X{} gate.

\begin{enumerate}
\item If the circuit is viewed as implementing an oracle, for what input string will the oracle return $1$?\Blank[3in]{}
\end{enumerate}
\item Show how we can use Shor's 3~qubit error correction scheme to address this problem only for the top qubit.
\LeaveSpace{3in}
\item Complete the circuit below so that each $c_i$ qubit is 0 if and only if $a_i$ has the same value as $b_i$.
\LeaveSpace{3in}
...
\begin{itemize}
   ...
   \item Deutsch--Jozsa solves a problem in polynomial time on a quantum computer that takes worst-case exponential time on a classical computer.~\TF{}
    ...
    \item   You agree to complete the evaluation by May~9.~\TF{}
\end{itemize}
