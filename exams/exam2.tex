...
\begin{enumerate}
    ...
    \]
    \item\Points{3} Below draw the quantum circuit that creates the above state, using only Hadamard and Pauli gates.  As usual, the left qubit is $q_0$ and the right qubit is $q_1$. 
...
\end{tabular}\end{center}
    ...
    \item\Points{2} If we perform phase estimation as taught in class (slide deck 30, ``Inverse Fourier Transform''), what value will we obtain for~$w$ as a binary fraction?{\Huge \ .\ }\Blank[1.7em]\ \Blank[1.7em]{}
...
If the error occurs, what is the resulting tensor-factored state?
    ...
    \]
\item\Points{5} If we perform phase estimation on $\ket{\psi_{e}}$, what will the result be? Show and explain your work clearly and carefully for full credit.
\LeaveSpace{1in}
...
Fill in the table below to show the bases Bob must have used and the observations Bob must have seen, where that observation is relevant to finding the shared key.  You should have an entry for each row for Bob's \emph{Basis}.  However, where Bob's observation is irrelevant, leave that entry blank.
