...
There are 121 points available for this exam, but it will only be scored out of 100.  Extra points earned here will count toward your total exam grade.
...
\clearpage\item\Points{10} For the Mermin--Peres square problem, suppose Alice and Bob agree before they separate that
\begin{itemize}
    \item Alice will follow the protocol exactly as taught in class.
    \item Bob will follow the protocol as taught in class, except that for each square in his column, he will negate the result he should have reported.   Where he should report $+1$, he would report $-1$;  where he should report $-1$ he will report $+1$.
\end{itemize}
With this change in protocol
\begin{itemize}
    \item The values in Alice's assigned row will continue to multiply to $+1$.~\TF{}
    \item The values in Bob's assigned column will continue to multiply to $-1$.\TF{}
    \item In the shared square for Alice and Bob, they will continue to report the same value.~\TF{}
    \item In the shared square for Alice and Bob, they are guaranteed to report different values.~\TF{}
\end{itemize}

\item\Points{10} Recall the Mermin--Peres square below.

\begin{tabular}{cc}
\begin{minipage}[t]{2.8in}\includegraphics[scale=0.6]{mp.png}\end{minipage} &
\raisebox{1in}{\begin{minipage}[t]{3in}\raggedright If Alice measures her qubits as $\ket{00}$ and Bob measures his qubits as $\ket{0+}$, then what results do they report for row and column~1, respectively?\end{minipage}}
\end{tabular}
...
In the above scenario, it is possible that Alice measures $\ket{00}$ and Bob measures $\ket{1+}$~\TF{}
...
\item\Points{10} Using Shor's algorithm, let's try to factor the number~24 using the following table of values for
\[ 17^{i} \mbox{ mod 24} \]
\begin{center}
\begin{tabular}{c|c}
