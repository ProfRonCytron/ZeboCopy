...
\clearpage\item\Points{20} Alice and Bob prepare to participate in one of the
many schemes we've studied this semester in which they begin with the 
Bell (also called an EPR) state:
\[ \frac{\ket{00}+\ket{11}}{\sqrt{2}} = \frac{1}{\sqrt{2}} \begin{pmatrix} 1\\0\\0\\1\end{pmatrix}\]
...
\begin{enumerate}
\item Suppose they each measure $\ket{\psi}$ in the computational (\textbf{Z})
basis.
...
\[
\frac{\ket{00}+\ket{11}}{\sqrt{2}} = \frac{\ket{++}+\ket{--}}{\sqrt{2}}
\]
by showing the expressions are mathematically equal:
\[
...
\]
After separation, if they each measure their qubit in the
\textbf{X} basis, which measurement outcomes are possible?
\GenTable{+}{-}
\Continued{}
...
to the state $\frac{1}{\sqrt{2}}\begin{pmatrix}1\\0\\0\\1\end{pmatrix}$.
In other words, Alice applied \textbf{H} to her qubit while Bob does nothing
to his.
The state resulting from that operation is:

\[ \ket{\psi_{2}} = \EmptyFour{} \]

Alice and Bob each measure their qubit of $\ket{\psi_{2}}$ in the computational
basis.  Which outcomes are possible?
\GenTable{0}{1}
Finally, they undo their work by reapplying
$\textbf{H}\otimes\textbf{I}$ to the measured outcome.

Which of the following states are possible now?
\GenTableDiff{+}{-}{0}{1}
\end{enumerate}
...
{\small
\[
U = \Blank[3em] \times \begin{pmatrix}
