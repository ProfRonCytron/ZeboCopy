...
\begin{itemize}
    ...
    \item Measurement operators are necessarily unitary. \TF{}
\end{itemize}
...
\begin{itemize}
    \item As evident on the Block sphere, the number of real parameters necessary to describe an arbitrary state of a single qubit is~\Blank{}.
    ...
    \item Consider a state $\psi$ on the Block sphere which is an eigenstate of some unitary gate~$U$.  The minimum nonnegative angular distance between~$\psi$ and~$U(\psi)$ on the Block sphere is~\Blank{}~radians.
    \item $\X\ket{0}=\BlQb$
    \item $\Z\ket{+}=\BlQb$
    \item $\H{\ket{0}}=\BlQb$
...
\clearpage\item\Points{10} Block Sphere Orienteering:  all answers are in the standard basis.  Write each amplitude in the provided blank space, fully above the dark line.  For example, to express $\frac{i}{\sqrt{3}}$ you would write \Blank{}\hbox to 0pt{\hskip -4em\raisebox{4pt}{$i/\sqrt{3}$}\hss}.
...
\clearpage\item\Points{10}  Suppose Alice and Bob decide to form a shared key using the BB84 protocol.  The table below shows the bases they will publish along with their observed results.
