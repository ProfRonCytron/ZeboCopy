...
\begin{itemize}[<+->]
    ...
    \item The bottom qubit's output has a constant value, regardless of $f(x)$.
...
\Vskip{-3em}\begin{itemize}
    ...
    \item<22-> For example, the sign can impose a phase change relative to the \QZero{} and \QOne{} terms for a single qubit.
\end{itemize}
}
...
\end{align*}
\visible<14->{%
$f(x)$ induces a signature of signs on the output basis states.}
}
}

\end{frame}

\section*{Controlled-U gate}
...
\only<1-4>{%
\begin{itemize}[<+->]
    \item The top qubit is $\ket{+}=\Hadamard\QZero{}=\TwoSupOp{\ColorOne{\QZero}}{\ColorTwo{\QOne}}{+}$.
    \item The bottom qubit is in state \ColorThree{\QState{}}, which is an eigenstate of the arbitrary unitary gate~\ColorFour{\NamedGate{U}}.
    \MedSkip{}
    We thus have
    \(\ColorFour{\NamedGate{U}}\ColorThree{\QState{}} = \lambda \ColorThree{\QState{}} \)
    \item This implies $\Mag{\lambda}=1$ so the result can be written:
    \[ \ColorFour{\NamedGate{U}}\ColorThree{\QState{}} = \ColorFour{\ExpPhase{\theta}}\ColorThree{\QState{}}\]
    with \ColorFour{$\ExpPhase{\theta}$}  as a \emph{global phase} on state~\ColorThree{\QState{}}.
    \item We return to this setting when we consider the \href{https://en.wikipedia.org/wiki/Quantum_counting_algorithm}{Quantum Counting Algorithm}.
\end{itemize}}%
\ScrollProof{5}{8}{%
\Next{\Four}{\QState{0} &= \TensProd{\ket{+}}{\QState{}}\\
&= \TensProd{\TwoSupOp{\ColorOne{\QZero}}{\ColorTwo{\QOne}}{+}}{\ColorThree{\QState{}}}\\}
\Next{\Three}{&= \TwoSupOp{\ColorOne{\QZero}\ColorThree{\QState{}}}{\ColorTwo{\QOne}\ColorThree{\QState{}}}{+} \\}
\Last{\QState{1} &= \TwoSupOp{\ColorOne{\QZero}\ColorThree{\QState{}}}{\ColorTwo{\QOne}\ColorFour{\NamedGate{U}\ColorThree{\QState{}}}}{+}}
}%
\ScrollProof{9}{12}{%
\Next{\Four}{\QState{1} &= \TwoSupOp{\ColorOne{\QZero}\ColorThree{\QState{}}}{\ColorTwo{\QOne}\ColorFour{\NamedGate{U}\ColorThree{\QState{}}}}{+} \\}
\Next{\Three}{&= \TwoSupOp{\ColorOne{\QZero}\ColorThree{\QState{}}}{\ColorTwo{\QOne}\ColorFour{\ExpPhase{\theta}}\ColorThree{\QState{}}}{+} \\}
\Next{\Two}{&= \TwoSupOp{\ColorOne{\QZero}\ColorThree{\QState{}}}{\ColorFour{\ExpPhase{\theta}}\ColorTwo{\QOne}\ColorThree{\QState{}}}{+} \\}
\Next{\One}{&= \TensProd{\TwoSupOp{\ColorOne{\QZero}}{\ColorFour{\ExpPhase{\theta}}\ColorTwo{\QOne}}{+}}{\ColorThree{\QState{}}}}
}%
\only<13->{%
\[
\QState{1} = \TensProd{\TwoSupOp{\ColorOne{\QZero}}{\ColorFour{\ExpPhase{\theta}}\ColorTwo{\QOne}}{+}}{\ColorThree{\QState{}}}
\]
\only<14-18>{%
\begin{itemize}
    \item<14-> The global phase \ColorFour{$\ExpPhase{\theta}$} on the bottom qubit has been kicked back (up) to the top qubit.
    \item<15-> It now manifests as a local phase on \ColorTwo{\QOne} as compared with \ColorOne{\QZero}.
    \item<16-> Further processing can approximate or perhaps identify~\ColorFour{$\theta$}.
    \item<17-> For example, if we know that either
    \begin{description}
       \item[$\theta=0$] then $\QState{1}=\TensProd{\ket{+}}{\ColorThree{\QState{}}}$
       \item[$\theta=\pi$] then $\QState{1}=\TensProd{\ket{-}}{\ColorThree{\QState{}}}$
    \end{description}
    \item<18-> Applying \Hadamard{} to the top qubit determines \ColorFour{$\theta$}.
\end{itemize}}%
\only<19->{%
\begin{itemize}
    \item<19-> Consider $0\leq\ColorFour{\theta}\leq \pi$.
    \item<20-> The top qubit in the \PauliX{} basis can be expressed as
    \[
    \cos\frac{\ColorFour{\theta}}{2}\ket{+} + \sin\frac{\ColorFour{\theta}}{2}\ket{-}
    \]
    \item<21-> So we measure~\ket{+} with probability~$\cos^{2}\frac{\ColorFour{\theta}}{2}$ and~\ket{-} with probability~$\sin^{2}\frac{\ColorFour{\theta}}{2}$.
    \item<22-> Assuming the circuit can be reconstructed and measured an arbitrary number of times, we can estimate \ColorFour{$\theta$} with arbitrary precision.
\end{itemize}
}
}
}{%
...
\adjustbox{scale=1.0}{\begin{quantikz}
    \lstick{\ket{+}} & \qw\slice{\alert<5-7>{\QState{0}}} & \ctrl{1}\slice{\alert<8->{\QState{1}}} & \qw & \qw \\
    \lstick{\ColorThree{\QState{}}} & \qw & \gate{\ColorFour{\NamedGate{U}}} & \qw & \qw
    \end{quantikz}}
\end{center}
\visible<4->{\[ \ColorFour{\NamedGate{U}\ColorThree{\QState{}}} = \ColorFour{\ExpPhase{\theta}}\ColorThree{\QState{}}\]}%
\visible<22->{%
We will study a more rigorous approach to phase estimation.}
}
