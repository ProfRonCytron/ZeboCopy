\SetTitle{26}{Grover's algorithm, part 1}{Unstructured search in $\sqrt{N}$ time}{26}
...
\MedSkip{}
Note that \W{} and \R{} are conceptual bases used to describe this space.  We cannot create either state \textit{a priori}. Starting at \S{}, we reach \W{} using Grover's algorithm.
}
\end{frame}
\end{Grover}
...
\Vskip{-3em}\begin{align*}
    ...
    & = \ColorOne{\RootTwoN{2}\CDQB{1}{1}{1}{1}}\times\ColorTwo{\RootN{3}\DQB{1}{1}{0}{1}}\\
    &  =\frac{\sqrt{3}}{2}\\[2em]
    \theta &= \frac{\pi}{6}
    }
\end{align*}}%
...
\item<7-> The second step reflects the current state \ColorFour{\QState{f}} around \S{} to obtain \ColorFive{\QState{d}}.
...
\Vskip{-3em}\begin{itemize}
    ...
    \item $\TensSupProd{\Hadamard}{2}\ColorSix{(t)}$ yields $\W=\Conj{\CDQB{0}{0}{1}{0}}\ColorFive{=\QState{d}}$
\end{itemize}
}
}{%
\Vskip{-4.8em}\hskip -3em\begin{center}
\begin{TIKZP}[scale=2.35]
    \visible<1->{\draw[->,thick,\RCone] (0,0) -- (30:1) node[right]{$\S = \RootTwoN{2}\DQB{1}{1}{1}{1}$};} 
    \visible<2-6>{\draw[->,thick,\RCthree] (0,0) -- (0,1) node[above right]{$\W=\Conj{\CDQB{0}{0}{1}{0}}\visible<7->{\ColorFive{=\BState{d}}}$} ;}
    \visible<7->{\draw[->,thick,\RCfive] (0,0) -- (0,1) node[above right]{$\W=\Conj{\CDQB{0}{0}{1}{0}}\visible<7->{\ColorFive{=\QState{d}}}$} ;}
    \visible<3->{\draw[->,thick,\RCtwo] (0,0) -- (1,0) node[right] {$\R=\RootN{3}\Conj{\CDQB{1}{1}{0}{1}}$};}
    \visible<4->{
...
\begin{Pixture}[width=0.8\textwidth]{260}{2qrealc.png}
\end{Pixture} \\
\begin{Pixture}[width=0.7\textwidth]{260}{2qrealp.png}
\end{Pixture}
\end{center}
}{%
...
\Vskip{-2em}\begin{center}
\begin{Pixture}[width=0.8\textwidth]{260}{4qrealc.png}
\end{Pixture} \\
\begin{Pixture}[width=0.7\textwidth]{260}{4qrealp.png}
\end{Pixture}
\end{center}
}
