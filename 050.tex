...
\begin{frame}{Quantum computation}{For simulating quantum systems and for implementing algorithms}

\alert{Want to draw a picture for this}
\begin{itemize}
    \item In this section we look at a \href{https://en.wikipedia.org/wiki/Mach-Zehnder_interferometer}{particular phenomenon} that can be explained using quantum mechanics.
    \item We can model such phenomena on a quantum computer. In this way, quantum computers simulate quantum systems from physics, chemistry, and other disciplines.
    \item Quantum computation and this course are more concerned with solving computational problems using quantum computers.
    \item It is useful, however, to look first at physical quantum phenomena and how they map to a quantum computation.
\end{itemize}
...
\begin{itemize}
    ...
    \item<2,4-> \textcolor{\RCone}{In the path shown here, the beam splitter does not reflect the photon.}
    \item<3,4-> \textcolor{\RCtwo}{In the path shown here, the photon is reflected by the beam splitter and again by the mirror.}
\end{itemize}
}{%
\begin{center}
\begin{TIKZP}[scale=0.75]
    ...
    \Shift{6}{0}{\Measurement[color=\RCone]{}}
    \Shift{6}{-2}{\Measurement[color=\RCtwo]{}}
    \visible<2,4->{\draw[thick,color=\RCone] (4.5,0.5) -- (6,0.5);}
    \visible<3->{\draw[thick,color=\RCtwo] (4.5,0.5) -- ++(0,-2) -- ++(1.5,0);}
...
\Vskip{-2em}\begin{itemize}
    \item<3-> Let \textcolor{\RCone}{$\QZero{} = \PZero{}$} be \textcolor{\RCone}{one path}.
    \item<3-> Let \textcolor{\RCtwo}{$\QOne{} = \POne{}$} be \textcolor{\RCtwo}{the other}.
...
\[
\alpha\textcolor{\RCone}{\ket{0}} + \beta\textcolor{\RCtwo}{\ket{1}} = \SQB{\alpha}{\beta}
\]
where $\alpha$ and $\beta$ characterize the \emph{presence} of \textcolor{\RCone}{\ket{0}} and \textcolor{\RCtwo}{\ket{1}}, respectively.  This is related to the probability of outcome, but there is more to say about that later.
}
}
\end{frame}
...
\begin{TIKZP}[scale=0.75]
    ...
    \Shift{6}{0}{\Measurement[color=\RCone]{}}
    \Shift{6}{-2}{\Measurement[color=\RCtwo]{}}
    \Shift{6}{-4}{\Measurement[color=\RCthree]{}}
    \visible<2->{\draw[thick,color=\RCone] (2.5,0.5) -- (4.5,0.5);}
    \visible<5->{\draw[thick,color=\RCone] (4.5,0.5) -- (6,0.5);}
    \visible<6->{\draw[thick,color=\RCtwo] (4.5,0.5) -- ++(0,-2) -- ++(1.5,0);}
    \visible<3->{\draw[thick,color=\RCthree] (2.5,0.5) -- ++(0,-4) -- ++ (3.5,0);}
    \draw[thick,color=red] (1,0.5) -- ++(1.5,0);

\end{TIKZP}
\end{center}
\only<3>{\textcolor{\RCthree}{There is thus a 50\% chance of hitting the bottom measuring device.}
}
\only<5>{\textcolor{\RCone}{There is a 25\% chance of hitting the top measuring device.}}
\only<6>{\textcolor{\RCtwo}{There is a 25\% chance of hitting the middle measuring device.}}
}
...
\TwoColumns{%
    ...
    \visible<1,4->{\textcolor{\RCone}{$\QZero{} = \begin{pmatrix}1\\ 0\\ 0\end{pmatrix}$}}
    \visible<2,4->{\textcolor{\RCtwo}{$\QOne{} = \begin{pmatrix}0 \\ 1\\ 0\end{pmatrix}$}}
    \visible<3,4->{\textcolor{\RCthree}{$\ket{2} = \begin{pmatrix}0 \\ 0\\ 1\end{pmatrix}$}}
...
\begin{itemize}
    ...
    \[
    \alpha\textcolor{\RCone}{\ket{0}} +
    \beta\textcolor{\RCtwo}{\ket{1}} +
    \gamma\textcolor{\RCthree}{\ket{2}} = \begin{pmatrix} \alpha \\ \beta \\ \gamma \end{pmatrix}
    \]
\end{itemize}
...
\begin{TIKZP}[scale=0.75]
    ...
    \Shift{6}{-2}{\Measurement[color=\RCtwo]{}}
    \Shift{6}{-4}{\Measurement[color=\RCthree]{}}
    \visible<1->{\draw[thick,color=\RCone] (2.5,0.5) -- (4.5,0.5);}
    \visible<1->{\draw[thick,color=\RCone] (4.5,0.5) -- (6,0.5);}
    \visible<2->{\draw[thick,color=\RCtwo] (4.5,0.5) -- ++(0,-2) -- ++(1.5,0);}
    \visible<3->{\draw[thick,color=\RCthree] (2.5,0.5) -- ++(0,-4) -- ++ (3.5,0);}
    \draw[thick,color=red] (1,0.5) -- ++(1.5,0);
...
\visible<2>{\begin{scope}[draw=\RCtwo,rotate=90]\PFilter{-1}{-1.2}{1}{1.2}{20}\end{scope}}
\visible<3>{\begin{scope}[draw=\RCthree]\PFilter{-1}{-1.2}{1}{1.2}{20}\end{scope}}
\end{TIKZP}
\end{center}
}
...
\begin{itemize}
  \item Surprisingly, \emph{every} photon is measured only by the \textcolor{\RCone}{device on the right}.
...
\end{itemize}}%
\only<6->{%
\begin{Pixture}[width=0.9\textwidth]{050}{phaseshift.png}
...
\begin{itemize}
  \item<1-> A photon is measured \textcolor{\RCtwo}{here} using one of two paths:
  \begin{itemize}
      ...
      \item<3-> \textcolor{\RCthree}{Here, the photon is reflected by each beam splitter.  \alert<3>{The accumulated delay is therefore $2\times \pi/2 = \pi$~radians.}}
  \end{itemize}
  \item<4-> \textcolor{\RCtwo}{Where the paths converge, the waves cancel, so there is no probability of measurement here.}
  \item<5> \textcolor{\RCone}{All photons are measured here.}
\end{itemize}
}{%
\Vskip{-2em}
\begin{center}
...
\draw[color=\RCthree] (2.5,0.5) -- ++(0,-2) -- ++(4,0);
\draw<3>[ultra thick,color=\RCthree] (2.5,0.5) -- ++(0,-2) -- ++(4,0) -- ++(0,-1.5);
\draw<1,5->[->,color=\RCone] (6.5,-1.5) -- ++(1.5,0);
\draw<1-4>[->,color=\RCtwo] (6.5,-1.5) -- ++(0,-1.5);
\end{TIKZP}
\MedSkip{}
...
\begin{itemize}
    \item Treatments that may lead you to think quantum mechanics can cause time to go backwards.
...
\end{itemize}
\item \href{https://en.wikipedia.org/wiki/Sabine_Hossenfelder}{Sabine Hossenfelder} makes this so clear.
\begin{itemize}
    \item \href{https://www.youtube.com/watch?v=RQv5CVELG3U}{Debunking delayed choice}
    \item Her amazing \href{https://www.youtube.com/channel/UC1yNl2E66ZzKApQdRuTQ4tw}{YouTube channel}
    \item Her \href{https://backreaction.blogspot.com/}{blog}
\end{itemize}
\end{itemize}
\end{frame}
...
\def\ralphap{\ensuremath{\textcolor{\RCone}{\alpha'}}}
\def\bbetap{\ensuremath{\textcolor{\RCtwo}{\beta'}}}
...
\visible<1->{Suppose the beam splitter is presented with a photon in superposition
    ...
    \begin{eqnarray*}
     ...
     \ralphap{} & = & \frac{1}{\sqrt{2}}\left(\visible<6->{\ralpha{}}
        + \visible<7->{\NiceI\bbeta{}}\right) \\
    \visible<8->{\bbetap{} & = & \frac{1}{\sqrt{2}}\left(\visible<9->{\NiceI\ralpha{}}
...
\begin{TIKZP}[scale=2.5]
    ...
    \visible<3->{\draw[->,color=\RCone] (0.5,0.5) -- (1.0,0.5) node[right] {$\alpha'$};}
    \visible<4->{\draw[->,color=\RCtwo] (0.5,0.5) -- ++(0,-0.5) node[below] {$\beta'$};}
    
\end{TIKZP}
\end{center}
\only<3-4>{We want to compute the effect of the beam splitter in creating a superposition of outputs: \textcolor{\RCone}{photon traveling east} and \visible<4->{\textcolor{\RCtwo}{photon traveling south}}}
...
\Vskip{-2em}\begin{itemize}
    ...
    \[ \BSG{} \SQB{1}{0} = \frac{1}{\sqrt{2}} \SQB{1}{\NiceI}
    \]}
    ...
    \[
    \XMatrix{} \RootTwo{}\SQB{1}{\NiceI} = \RootTwo{}\SQB{\NiceI}{1}
    \]}
    \only<4-5>{\item The final beam splitter produces
    \[
    \BSG{}\RootTwo{}\SQB{\NiceI}{1} = \frac{1}{2}\SQB{2\NiceI}{\NiceI^{2}+1} = \SQB{\NiceI}{0}
    \]}
    ...
    \item
    We can also see the top component is rotated by $\pi/2$ radians.  The superpositions measured \textcolor{\RCone}{by this device} each experienced one beam-splitter reflection.}
\end{itemize}
}{%
\begin{center}
    ...
    \visible<1->{\draw[->,color=\RCone] (0.5,0.5) -- (1.0,0.5) node[right] {$\alpha'$};}
    \visible<1->{\draw[->,color=\RCtwo] (0.5,0.5) -- ++(0,-0.5) node[below] {$\beta'$};}
