...
\Vskip{-3em}\begin{itemize}
    ...
    \item<3-> What happens the photons strike a filter, going into the screen, as shown here?
...
\begin{itemize}
    ...
    \item The energy to push against media to create a wave of amplitude $A$ is computed using hooks law: \[  F = -k\times x \] where $x$ is the displacement.
    \item To achieve amplitude $A$ we must push continually from $0$ to $A$, so that the total energy is 
    \[
    \int_{0}^{A} kx \, dx = k\frac{A^{2}}{2}
    \]
\end{itemize}
}{%
\begin{center}
