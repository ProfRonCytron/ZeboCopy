...
\begin{itemize}
    ...
    \begin{description}
        \item[sunglasses] These are polarized to cut down on reflected light from surfaces such as water.  Reflected light is typically polarized horizontally, so these sunglasses admit only vertically polarized light.
    ...
    \item In the slides that follow, light is emitted from the screen and heading toward you.    Each successive filter is placed in front of the previous one.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item The energy in a wave is a function of its amplitude.
    \item The energy to push a spring distance $x$ is: \[  F = -k\times x \] for some constant $k$ (\href{https://en.wikipedia.org/wiki/Hooke\%27s_law}{Hooke's law}).
    \item To achieve amplitude $d$ we must push continually from $0$ to $d$, so that the total energy is 
    \[
