...
\begin{frame}{Achieving rotation}{The Bloch sphere and the physical rotation of a polarizing filter}
\TwoUnequalColumns{0.65\textwidth}{0.35\textwidth}{%
With no bomb present
\begin{itemize}
    \item<1-> The polarization of the photon rotates from
    \raisebox{-1em}{\begin{tikzpicture} \HPolar{}\end{tikzpicture}}
    to \begin{tikzpicture} \VPolar{}\end{tikzpicture}
    \item<2-> A physical rotation of $\pi/2$ radians.
    \item<3-> A state change from \ket{0} (horizontal) to \ket{1} (vertical).
    \item<4-> On the Bloch sphere, the same rotation is $\pi$ radians, from the north to south poles.
    \item<5-> To accomplish the rotation on the Bloch sphere in $n$ steps, each step must rotate by
    $\theta=\pi/n$ radians.
\end{itemize}
}{%
\only<1-2>{%
