...
\begin{enumerate}
    ...
    \item<14-> Repeat the above two steps until $n$ rotations have been achieved.
    \item<15-> Measure after $n$ rotations.
\end{enumerate}
}
}{%
...
%%
%% Bounding box
\draw[gray] (0,1) rectangle ++(6,-4.5);
%%
\Shift{0}{-1}{%
\begin{scope}
\fill<1,13->[yellow] (2.5,-2.5) rectangle (4.5,1.5);
\visible<2,5-12>{\HPolarizedLightSource{}}
\visible<3>{\VPolarizedLightSource{}}
...
\only<15>{%
\MedSkip{}
We will properly determine $\theta$ so that after $n$ rotations we either will see \ket{0} if the bomb package is there, or \ket{1} if it is absent.  And each time through the package, we will have a predictably low (though always positive) chance of blowing up.
}
\end{frame}
