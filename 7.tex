...
\begin{itemize}
    \item<4-> Each approach to the bomb blows up with probability $\sin^{2}(\pi/2n)$.
    \item<5-> In radians \Implies{\sin x \leq x}{\sin^{2}x \leq x^{2}}
    \item<6-> The probability of at least one explosion in $n$ tries is therefore bounded by
    \[
        n\left(\frac{\pi}{2n}\right)^{2} = \frac{\pi^2}{4n} \leq \frac{2.5}{n}
    \]
    \item<7-> The chance of blowing up can be made arbitrarily small.
\end{itemize}
}
\end{frame}
