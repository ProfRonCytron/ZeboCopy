...
\begin{itemize}
    \item<1-> With polarizing filters, our bases were horizontal \visible<2->{and vertical, so they were separated by \Degrees{90}.}
...
\begin{frame}{A note on our \href{https://en.wikipedia.org/wiki/Spherical_coordinate_system}{spherical coordinate system}}{Physics and math have different conventions.  \Quote{Why, people, why?} -- Professor Brian Garnett, 2024}
\Vskip{-5em}\TwoColumns{%
\begin{center}
\begin{Pixture}[height=8em]{060}{physics.png}
\end{Pixture}\end{center}
This is the convention followed by physics.  \alert{Because most texts depicting the Bloch sphere use this convention, so shall we.}
}{%
\begin{center}
\begin{Pixture}[height=8em]{060}{math.png}
\end{Pixture}\end{center}
This is the convention followed by math.  You have likely seen these symbols for describing spherical coordinates in your math courses.
}
\BigSkip{}
\begin{center}For quantum computing, typically $r=1$\end{center}
\end{frame}
...
\]
\begin{itemize}
    \item<1-> This transformation exchanges the amplitudes and imposes a relative sign change (combines \PauliX{} and \PauliZ{}): \[ \ApplyGate{\PauliY}{\SQB{\alpha}{\beta}} =  \SQB{-\NiceI\beta}{i\alpha} = -\NiceI\SQB{\beta}{-\alpha} \visible<2->{\alert{\equiv} \SQB{\beta}{-\alpha}} \]

    \item<3-> The two eigenstates are \ket{+y} and \ket{-y}. \Exercise{unaffected points}{Define +y and -y in the standard basis and show that Paul Y doesn't affect +y or -y.}
    \item<4-> This gate maps $\ket{0}\mapsto\ket{1}$ and \VV{}.
\end{itemize}
}{%
\Vskip{-3em}
\begin{center}
...
\visible<2->{\MedSkip{}The ``\alert{$\equiv$}'' notation means equal up to a global phase.}
\visible<4>{\MedSkip{}This gate also maps $\ket{+} \mapsto \ket{-}$ and \VV{}.}
}
\end{frame}

\section*{Pauli Properties}
...
\begin{center}\bf Quantum Gates\end{center}
\begin{itemize}
    \item The \PauliZ{} gate sends an arbitrary quantum state to another state, as computed by the matrix:
    \[ \ApplyGate{\ZMatrix}{\SQB{\alpha}{\beta}}=\SQB{\alpha}{-\beta}
    \]
    \item The state after the gate is simply \[\SQB{\alpha}{-\beta}\]
\end{itemize}
}{%
\begin{center}\bf Measurement Operators\end{center}
\begin{itemize}
    \item Given any input state $\SQB{\alpha}{\beta}$, \PauliZ{} as a measurement operator yields one of its eigenstates, namely \PZero{} or \POne{}.
    \item This is clearly and intentionally not reversible.  More about this on the next slides.
\end{itemize}
}
\end{frame}
...
\only<13->{%
\begin{alignat*}{5}
    \XMatrix{} & \alert<14>{\PPlus{}} & \mbox{ = } & +1  & \alert<14>{\PPlus{}}  \\
    \visible<15->{\XMatrix{} & \alert<15>{\PMinus{}} & \mbox{ = } & -1  & \alert<15>{\PMinus{}} }
\end{alignat*}}
\begin{center}
\begin{TIKZP}
\visible<8->{
...
\end{frame}
\section*{Measurement in other bases}
\begin{frame}{Measurement in other bases}{Of the Bloch sphere}
\TwoColumns{%
\only<1>{%
\begin{itemize}
    \item Qiskit and actual quantum devices offer measurements in only one basis, the \PauliZ{} basis.
    \item Proximity of a state to the North or South poles results in \ket{0} or \ket{1} as a result, respectively. 
    \item If we pick another basis, say \PauliX{}, then on the Bloch sphere, the resulting measurement is based on state proximity to \ket{+} or \ket{-}.
\end{itemize}}%
\only<2>{%
\begin{itemize}
    \item We can measure in the \PauliX{} basis by first rotating the globe so \ket{+x} is at the North pole and \ket{-x} is at the South pole.
    \item Then we do a measurement as usual, in the \PauliZ{} basis.
    \item The result will be \QZero{} or \QOne{}, the North or South pole, respectively.
    \item We then unrotate the globe back to its original position, with the current state now \ket{+} or \ket{-}.
\end{itemize}
}
}{%
\begin{center}
\begin{BlochSphere}[scale=1.8]{60}{115}
    \BlochSpherePhiTheta{}
\end{BlochSphere}
\end{center}
}
\end{frame}
\begin{frame}{Measuring in non-\PauliZ{} basis}{Managing the transformations}
\Vskip{-4em}
\TwoColumns{%
\only<1-2>{%
\begin{itemize}
    \item First, conceive of the unrotation, namely mapping \PauliZ{} to your desired basis.  You must know where to send \ket{0} and \ket{1}:
    \[\PZero{}\mapsto\ColorOne{\SQB{\alpha}{\beta}}\ \POne{}\mapsto\ColorTwo{\SQB{\gamma}{\delta}}
    \]
    \item The matrix to accomplish this mapping has each of those states as its columns:
    \[
      T = \SQBG{\relax}{\ColorOne{\alpha}}{\ColorTwo{\gamma}}{\ColorOne{\beta}}{\ColorTwo{\delta}}
    \]
\end{itemize}}%
\only<3->{%
\begin{itemize}
    \item If $T$ unrotates out of the standard basis, then $\Inverse{T}$ performs the rotation we need \emph{into} the standard basis.
    \item For unitary matrices, $\Inverse{T}=\Conj{T}$.
    \item  Thus, if \[
      T = \SQBG{\relax}{\ColorOne{\alpha}}{\ColorTwo{\gamma}}{\ColorOne{\beta}}{\ColorTwo{\delta}}
    \] then
       \[
      \Conj{T} = \SQBG{\relax}{\ColorOne{\Conj{\alpha}}}{\ColorOne{\Conj{\beta}}}{\ColorTwo{\Conj{\gamma}}}{\ColorTwo{\Conj{\delta}}}
    \]
\end{itemize}
}
}{%
\begin{itemize}
    \item<1-> To unrotate from \PauliZ{} to \PauliX, sending \QZero{} to $\ket{+}$ and \QOne{} to $\ket{-}$, we use the familiar Hadamard matrix:
    \[ \Hadamard{}=\HMatrix{} \]
    \item<4-> To rotate from \PauliX{} to \PauliZ, we simply compute $\Conj{\Hadamard}=\Hadamard{}$.
    \item<4-> To summarize the general case, we apply~$\Conj{T}$, perform the measurement in the standard basis, and then apply~$T$.
\end{itemize}
}
\end{frame}
%%
...
\def\Term#1{\frac{\NiceI^{#1}\theta^{#1}A^{#1}}{#1!}}%
\def\TermA#1{\frac{\NiceI{}\theta^{#1}A}{#1!}}%
