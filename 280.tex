\SetTitle{28}{Introduction to Shor's algorithm}{The non-quantum part}{28}
...
\end{frame}
\begin{frame}{Overview using an example (\href{http://www.apfloat.org/calculator/}{calculator})}{Factoring 217 by finding the period of an apparently unrelated function}
\TwoColumns{%
\Vskip{-4em}\begin{center}
{\scriptsize $g(x) = \ColorFour{5}^{x}\mbox{ mod $217$}$}\\[-0.5em]
\begin{Pixture}[width=0.9\textwidth]{280}{base5mod217.png}
...
\begin{frame}{Factoring $N=217$ using a base $b=11$ (\href{http://www.apfloat.org/calculator/}{calculator})}{Throughout, \emph{abort} requires starting over with a new (guessed) value for the base}
