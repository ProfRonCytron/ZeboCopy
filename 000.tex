...
\begin{frame}{Typesetting of proofs}{Full-screen viewing \emph{really does} work best.  This example is courtesy of \href{https://chat.openai.com/}{ChatGPT}}
Theorem:  \[ \sum_{k=1}^{n} k = \frac{n\times(n+1)}{2}\]
\begin{Reasoning}
...
\end{Reasoning}
\ScrollProof{1}{5}{%
\Next{\Four}{S &= \ColorOne{1 + 2 + 3 + \ldots + n} \\}
\Next{\Three}{S &=  \ColorTwo{n+(n-1) + (n-2)\ldots 1 }\\}
\Next{\Two}{ S+S &= (\ColorTwo{n}+\ColorOne{1}) + (\ColorTwo{n-1}+\ColorOne{2}) + \ldots + (\ColorTwo{1}+\ColorOne{n}) \\}
...
\begin{frame}{Course outline}{Subject to change: topics in \alert{red} are likely pushed to the advanced course}
...
\begin{enumerate}
    ...
    \alert{\item Circuit compilation}
    \alert{\item Phase estimation}
    \alert{\item Quantum Fourier}
