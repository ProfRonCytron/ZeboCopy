...
\begin{itemize}
    ...
    \only<2>{
    \SmallSkip{}
    Well \emph{some people} do build such machines:
    \begin{itemize}
...
\begin{itemize}
    \item As with classical computing, we are interested in the computational complexity, expressed asymptotically, for problems and algorithms of interest.
    \item We study unstructured search later, but here we see the value of finding a needle in a large haystack.  
    \item Problems such as factoring and discrete log have impressive solutions using quantum computing, but we do not yet know a tight bound on the complexity of such problems using classical computation. 
\end{itemize}
