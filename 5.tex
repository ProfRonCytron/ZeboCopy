...
\item The explanation requires understanding phase and interference.   \item Mathematically this is easiest with complex arithmetic.
\end{itemize}
}{%
\begin{center}
...
\begin{frame}{Analysis of the photon's paths}{The bottom measuring device}
...
\begin{itemize}
  ...
  \begin{itemize}
      \item<2-> Here, the photon is reflected by a mirror, but passes through both beam splitters.  \alert<2>{There is no phase shift in this case.}
      \item<3-> Here, the photon is reflected by each beam splitter.  \alert<3>{The accumulated delay is therefore $\pi$~radians.}
  \end{itemize}
  \item<4-> Where the paths converge, the waves cancel, so there is no probability of measurement \textcolor{NavyBlue}{here}.
  \item<5> All photons are measured \textcolor{orange}{here}.
\end{itemize}
}{%
\Vskip{-2em}
\begin{center}
...
We next look at how to model this behavior mathematically.}
}
\end{frame}
