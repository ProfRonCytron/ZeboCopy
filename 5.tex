...
\begin{itemize}
    ...
    \item<3-> With equal probability, \alert<3>{yet utter unpredictability}, a beam splitter takes a photon and either
    \begin{itemize} 
...
\begin{itemize}
    ...
    \item To that end, we study here the \href{https://en.wikipedia.org/wiki/Mach-Zehnder_interferometer}{Mach--Zehnder interferometer}. It looks like a photon should be able to reach either measuring device.
\end{itemize}
...
\end{center}
\visible<5>{%
When reflected, we assume here that the photon experiences a delay of $\frac{\pi}{2}$ radians compared with passing through the beam splitter.
}
}
...
\begin{frame}{Show the quantum eraser here?}{Not sure.}
\end{frame}
...
\begin{itemize}
    ...
    \item Suppose we begin with an amplitude $\alpha$, which already may embody a phase shift.
...
\begin{itemize}
    ...
    \item<3-> We know we want the probability of measuring either outcome to be $\frac{1}{2}$.
...
\only<5-7>{\ralphap{} is composed of an equal fraction of \only<6-7>{\ralpha{} continuing right} \only<7>{ and \bbeta{} bending to the right to reflect.}}
\only<8-10>{\bbetap{} is composed of an equal fraction of \only<9-10>{\ralpha{} bending down to reflect} \only<10>{ and \bbeta{} continuing down.}}
}%
\only<11->{%
...
\]
and we view the beam splitter as represented mathematically by
\[
\BSG{}
\]
}
...
\Vskip{-2em}\begin{itemize}
    ...
    \item
    We can also see the top component is rotated by $\pi/2$ radians.  The superpositions that are measured \textcolor{orange}{by this device} each experienced one beam-splitter bend.}
\end{itemize}
}{%
\begin{center}
...
\begin{itemize}
    \item We express superpositions using wave amplitudes that are complex numbers.
    \item Complex numbers allow us to capture both the magnitude of the wave as well as its phase.
    \item We can model the effects of a quantum gate using a square matrix.
    \item Such matrices have other properties we will soon study.
\end{itemize}
