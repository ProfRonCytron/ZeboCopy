...
\begin{itemize}
    ...
    \item Gears, sprockets, and springs
    \item Water
    \item DNA
    \item \alert{Gravity}
...
\begin{frame}{Using DNA to Solve Traveling Salesperson}{By ligation and electopheresis}
    
\begin{columns}
    \begin{column}{0.5\textwidth}
        \begin{itemize}
            \item Each city is represented by a strand of DNA, with a unique first and last part.
            \item For each edge in the graph, it is possible that the strand of one city will ligate to the strand of the other city, thus extending the path
        \end{itemize}
 
    \end{column}
    \begin{column}{0.5\textwidth}  %%<--- here
     \begin{center}
        \includegraphics[width=\textwidth]{0/map.jpeg}
     \end{center}
    \end{column}
\end{columns}
...
These strands of DNA can be sorted by length using electropheresis, with the shortest acceptable strand encoding the solution.

{\tiny \url{https://www.jyi.org/2005-september/2005/9/7/fear-not-traveling-salesmen-dna-computing-is-here-to-save-the-day}}
\end{frame}