...
\only<2->{We know
\begin{eqnarray*}
\ColorR{d} & = & \frac{1}{2} a \ColorG{t}^{2} \\
\ColorG{t} & = & \sqrt{\frac{2\ColorR{d}}{a}}
\end{eqnarray*}}
\only<3->{To compute $\sqrt{n}$, we just need to drop a ball from height $\ColorR{d}=\frac{an}{2}$ and measure the time for the ball to hit the ground.}
\end{frame}