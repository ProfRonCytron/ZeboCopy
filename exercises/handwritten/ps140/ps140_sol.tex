...
\assignment{Problem Set 140 Solution}
...
\end{itemize}
    ...
    What is the state of Bob's qubits? \Ans{Bob's qubits are in the same state.  There may be a local phase difference introduced, but likely the students won't know about that.}
    ...
    \Ans{\QPlusSym{} or \QMinusSym{}}
    
    \item \Points{10} Are all combinations of outcomes possible?  \Blank{}.  If not, say which are possible below
    
    \Ans{No, he can only see $\ket{0+}$ or $\ket{1-}$}
    \item \Points{10} Referring to Slide 14 in deck 140, what two possible states ($\psi_{?}$ or $\psi_{?}$) could Bob see when he measures after Alice?
    
    \Ans{$\QState{0}$ or $\QState{3}$}
    ...
    \begin{itemize}
        \item Top square \Ans{$+1$}
        \item Middle square \Ans{$+1$}
        \item Bottom square \Ans{$-1$}
    \end{itemize}
    \item \Points{10} What value must Alice have reported for the first square in her row in this scenario? \Ans{$-1$}
