...
\lstick{\QZero{}}\slice{\alert<1>{\QState{0}}} &  \gate{H}\slice{\alert<2>{\QState{1}}}  & \qw\\
\lstick{\QZero{}} &   \gate{H}    &  \qw
\end{quantikz}}
}{%
\Vskip{-3em}\begin{align*} 
\alert<1->{\QState{0}}= &\ket{00} 
...
\visible<3->{= & \frac{1}{\sqrt{2^{n}}}\ket{1^{n}}}
}
\end{align*}
\Vskip{-2em}\begin{itemize}
\item<1-> There are $n$ inputs, each \ket{0}.
\item<2-> We apply an $n$-way Hadamard, which can be realized by a separate Hadamard on each input.
\item<3-> The result is the uniform superposition of all $n$-bit basis vectors.  If we regard each of those vectors as the encoding of an integer in $\left[0, 2^{n}\right)$, then we can think of the result as the uniform superposition of all $2^{n}$ values.
\end{itemize}
}
\end{frame}